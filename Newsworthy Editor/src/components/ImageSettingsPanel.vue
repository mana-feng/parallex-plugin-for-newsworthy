...
<template>
  <div class="mt-4 rounded bg-white/70 p-3">
    ...
    <div class="setting-item" v-if="!isFullWidth">
      ...
      <input type="range" min="50" max="1200" step="10" :value="image.width ?? 300"
        @input="$emit('updateWidth', $event.target.value)" />
    </div>

    <div class="setting-item" v-if="!isFullWidth">
      <label>Height: {{ image.height ?? 300 }}px</label>
      <input type="range" min="50" max="1200" step="10" :value="image.height ?? 300"
        @input="$emit('updateHeight', $event.target.value)" />
    </div>

    <div class="setting-item" v-if="!isFullWidth">
      <label class="inline-flex items-center gap-2">
        <input type="checkbox" :checked="!!image.keepRatio" @change="$emit('updateKeepRatio', $event.target.checked)" />
        Keep Aspect Ratio
      </label>
    </div>
    ...
    <div v-if="isFullWidth" class="setting-item">
      ...
      <select :value="image.mode || 'auto'" @change="$emit('updateMode', $event.target.value)">
    ...
    <div v-if="isFullWidth && image.mode === 'fixed'" class="setting-item">
      ...
      <input type="range" min="120" max="1200" step="10" :value="image.height || 400"
        @input="$emit('updateHeight', $event.target.value)" />
    </div>
    ...
    <div class="setting-item">
      <label>Caption</label>
      <textarea rows="2" :value="image.caption || ''" placeholder="Enter caption (optional)..."
    ...
    <div class="setting-item">
      ...
      <select :value="image.captionPosition || 'bottom'" @change="$emit('updateCaptionPosition', $event.target.value)">
    ...
    <div class="setting-item" v-if="image.captionPosition === 'bubble'">
      <label class="inline-flex items-center gap-2 text-sm">
        <input type="checkbox" :checked="!!image.captionBubbleAnimated"
